...
describe ReservarPublicacion do
  ...
  it 'si es P2P levanta un error y no guarda nada' do
    expect{described_class.new.ejecutar(publicacion_p2p)}.to raise_error(PublicacionNoReservableError)
    expect(Persistence::Repositories::RepositorioPublicaciones.new.find(publicacion_p2p.id).estado).to eq EstadoActivo.new
  end
end
